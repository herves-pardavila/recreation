...
if __name__ =="__main__":
    ...
    df.loc[df.PUD.isin([2,3,4,5]),"PUD"]=2
    df.loc[df.PUD>5,"PUD"]=3
    df.PUD.hist()
    plt.show()
    ...
    modelo,train,test,rsq=train_random_forest(df,T=1000,rho=len(explanatory_variables)/3,D=20,training=0.7,variables=explanatory_variables,y_variable="PUD",bootstrap=0.9)
    conf_mat = confusion_matrix(test[variable_y], test["yhat"],normalize="true")
    print(conf_mat)
