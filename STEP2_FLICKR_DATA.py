...
import pandas as pd
import sys
sys.path.append("/media/david/EXTERNAL_USB/doctorado/OneDrive/recreation")
...
if __name__ == "__main__":
    ...
    con=FlickrPhotos(main_path+"/github/github/recreationdb/photos_parques_naturales.csv")
    #clip by mask of the area of interest
    con.clip(main_path+"/OneDrive/geo_data/protected_zones/parques_nacionales.shp")
    clipped_df=con.gdf
    clipped_df.to_file(main_path+"/recreation/flickr_photos.shp",index=False)
    ...
    print(pud)
    pud.to_csv(main_path+"/recreation/pud.csv",index=False)
