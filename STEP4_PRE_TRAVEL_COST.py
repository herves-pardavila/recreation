...
if __name__== "__main__":
    path=r"F:/doctorado/"
    #load the data
    df=pd.read_csv(path+"recreation/ZonalTravelCost/3travel_cost_Cabañeros.csv")
    #df=df[df.Año==2019]
    print(df)
    ...
    print(df.info())
    df["TC"]=df["CT_(€)"]+df["OC_(€)"]
    df.to_csv(path+"recreation/ZonalTravelCost/3travel_cost_Cabañeros_ready.csv",index=False)
    ...
    new_gdf.to_file(path+"recreation/ZonalTravelCost/3travel_cost_Cabañeros.gpkg",driver="GPKG")
